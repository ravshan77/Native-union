...
const ProductCorusel = () => {
  ...
  return (
    <section className="product background_f">
      ...
      <p className="Explore_title">{t("EXPLORE OUR")}</p>
      <div className="Explore_text_div">
        <p
          className={`${
            slideOne ? "Explore_text fontSize" : "Explore_text_of fontSize"
          }`}
          onClick={handleSlideOne}
        >
          {t("BEST SELICES")}
        </p>
        <p
          className={`${
            slideTwo ? "Explore_text fontSize" : "Explore_text_of fontSize"
          }`}
          onClick={handleSlideTwo}
        >
          {t("NEW RELEASES")}
        </p>
      </div>
      ...
      <div className="product_corusel">
        <div className={`scroll_corusel ${position}`}>
          {products.map((product, index) => {
            return (
              <div key={index} className="corusel_cards">
        ...
        <button
          className="arrows_btns product_prev"
          onClick={() => setIndex(index - 1)}
        >
          <NavigateBeforeIcon />
        </button>
        <button
          className="arrows_btns product_next"
          onClick={() => setIndex(index + 1)}
        >
          <NavigateNextIcon />
        </button>
